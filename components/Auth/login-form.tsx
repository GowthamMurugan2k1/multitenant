...
import { LoginUser } from "@/api-hub/user-auth";
...
export function LoginForm({
  ...
  const EmailLoginUser = useMutation({
    mutationKey: ["new-usercreation"],
    mutationFn: (data: UserAuthProp) => LoginUser({ data }),
    onSuccess: (successData: any) => {
      ...
      if (error === "User not found.") {
        toast.error(error);
        toast("Look's like you don't have an account.", {
          icon: "🤔",
        });
  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    EmailLoginUser.mutate(updatedObj);
            ...
            <div className="grid gap-6">
              ...
              <div className="grid gap-6">
                ...
                <Button
                  ...
                  disabled={EmailLoginUser.isPending}
                >
                  {EmailLoginUser.isPending ? "Loading.." : "Login"}
                </Button>
              </div>
